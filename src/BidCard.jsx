...
const BidCard = ({ Data, budget }) => {
  ...
  const finalizeBid = async () => {
    try {
      const response = await fetch(`http://localhost:5000/bids/finalize-bid/${Data._id}`, { method: "POST" });
      if (response.ok) {
        console.log("Bid finalized successfully!");
      } else {
        console.error("Failed to finalize bid");
      }   
    } catch (error) {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5000/api/finished-projects")
      .then((res) => res.json())
      .then((data) => setFinishedProjects(data))
      .catch((error) => console.error("Error fetching finished projects:", error));
      console.log('Finished Projects : ',finishedProjects);
    const savedLastBidder = localStorage.getItem(`lastBidder_${Data._id}`);
    if (savedLastBidder) {
      setLastBidder(savedLastBidder);
    }
    ...
    socket.on("noBids", ({ auctionId, lastBidder }) => {
  ...
  const resetBids = async () => {
    try {
      const response = await fetch("http://localhost:5000/reset-bids", { method: "POST" });
      if (response.ok) {
        setLowestBid(budget);
        setLastBidder(null);
        console.log("Bids reset successfully!");
      } else {
        console.error("Failed to reset bids");
      }
    } catch (error) {
